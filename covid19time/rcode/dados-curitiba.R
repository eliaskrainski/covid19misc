...
t3 <- table(dcwb$ENCERRAMENTO, dcwb$fdate)
str(t3)
...
(jj <- pmatch(c(##paste0('202011', 28:30),
     ...
     paste0('202101', sprintf('%02d', 8:12))), 
     colnames(t3)))
t3a[jj, ]
...
t3a[jj, ] <- cbind(
    ...
    c(##52084, 52238, 52484, 52704, 53342, 54013, 54695,
        ...
        106487, 106864, 107200, 107319, 108336))

if (!any(is.na(t3a[-1:0 + nrow(t3a), ])))
