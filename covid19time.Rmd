...
mu.names <- unique(as.character(wdl[[1]]$City)) 
```
...
renderPlot({
  ...
  if (sum(iis[iic])==0) iis <- iic
  mu.names <- unique(as.character(
        wdl[[k]]$City[which(iic & iis)]))
  iim <- wdl[[k]]$City==input$city
  if (sum(iim[iic])==0) iim <- iic
  iicsm <- which(iic & iis & iim)
