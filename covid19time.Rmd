...
renderPlot({
 ...
 if (FALSE) ### just for test in .R 
   input <- list(data=1, country='Brazil', state='PARANÁ', city='Curitiba/PR', log10=TRUE, dateRange=c('01/01/20', '01/09/20')) 
 ...
 #   validate(
  ...
  if (length(dtmp$x)>9) {
    sss[dtmp$x] <- gam(
      y ~ s(x), poisson(), data=dtmp)$fitted
...
#    ee[1:n0] <- (1-w)*dtmp$y[1:n0] * exp(-(1:n0))/exp(-1)
 #   for (i in 1:n1)
  #     ee[i+1:n0] <- ee[i+1:n0] + dtmp$y[i] * w 
   # dtmp$lE <- log(ifelse(ee<0.15, 0.15, ee)[1:n1])
  ...
  if (n.obs>9) { 
    ...
    abline(h=0, lty=2, col=gray(0.5, 0.5))
    plot(Date, Rt, axes=FALSE, type='l', 
    ...
    text(rep(tail(Date, 1)-1, 3), 
         c(tail(Rtlow, 1), tail(Rt,1), tail(Rtupp, 1)), 
         sprintf("%1.2f", c(tail(Rtlow, 1), tail(Rt,1), tail(Rtupp, 1))))
    axis(1, pretty(Date), format(pretty(Date), '%d %b'))
    axis(2, las=1)
