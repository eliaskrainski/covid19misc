...
renderPlot({
 ...
 #   validate(
  ...
  if (length(dtmp$x)>9) {
    ...
    dtmp2 <- list(y = tail(dtmp$y, n1-n0))
    dtmp2$x <- tail(dtmp$x, n1-n0)
    dtmp2$lE <- log(ifelse(ee<0.15, 0.15, ee)[n0+1:(n1-n0)])
    rt <- c(rep(NA, n0), 
            exp(predict(gam(y~s(x), poisson(), 
                        data=dtmp2, offset=lE))))
  } else {
