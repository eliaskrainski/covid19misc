...
renderPlot({
 ...
 #   validate(
  ...
  sss <- r
    if (length(dtmp$x)>9) {
      sss[dtmp$x] <- mgcv:::gam(y ~ s(x), poisson(), dtmp)$fitted
  } else {
      sss[dtmp$x] <- 0.5*dtmp$y + 0.5*mean(dtmp$y)
  }

  xlm <- as.Date(input$dateRange, '%d/%m/%y')
  if (diff(xlm)<3) stop(safeError('Selecione 3 ou mais dias!'))
  iip <- which((Date>=xlm[1]) & (Date<=xlm[2]))
  if (length(iip)<3) stop(safeError('Selecione 3 ou mais dias!'))
  Date <- Date[iip]
  y <- y[iip]
  dy <- dy[iip]
